...
class PeopleRepository(PeopleRepositoryInterface):
    ...
    def list_people(self) -> List[PeopleTable]:
        with self._db_connection as database:
            try:
                return database.session.query(PeopleTable).all()
            except NoResultFound:
                return []

    def delete_people(self, first_name: str, last_name: str) -> None:
        with self._db_connection as database:
            try:
                (
                    database.session.query(PeopleTable)
                    .filter(
                        PeopleTable.first_name == first_name,
                        PeopleTable.last_name == last_name,
                    )
                    .delete()
                )
                database.session.commit()
